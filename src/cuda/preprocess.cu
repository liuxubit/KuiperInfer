    ...
    if (tid < out_height * out_width){
        ...
        if (out_row < offset_row || out_row >= letter_height + offset_row ||
            ...
            *out_b = (kDefaultFillValue - mean[2]) /  std[1];;
            ...
            // float in_row_f = (float)(out_row - offset_row + 1) / ratio + 0.5;
            // float in_col_f = (float)(out_col - offset_col + 1) / ratio + 0.5;
            // float in_row_f = (float)(out_row - offset_row + 1) / ratio + 0.5;
            // float in_col_f = (float)(out_col - offset_col + 1) / ratio + 0.5;
            ...
            *out_r = ((float)(in_image[x_0_y_0 * 3 + 2] * W_x_0 * W_y_0 +
                        in_image[x_1_y_0 * 3 + 2] * W_x_1 * W_y_0 +
                        in_image[x_0_y_1 * 3 + 2] * W_x_0 * W_y_1 +
                        in_image[x_1_y_1 * 3 + 2] * W_x_1 * W_y_1) / 255.f - mean[0]) / std[0];

            *out_g = ((float)(in_image[x_0_y_0 * 3 + 1] * W_x_0 * W_y_0 +
                        in_image[x_1_y_0 * 3 + 1] * W_x_1 * W_y_0 +
                        in_image[x_0_y_1 * 3 + 1] * W_x_0 * W_y_1 +
                        in_image[x_1_y_1 * 3 + 1] * W_x_1 * W_y_1) / 255.f - mean[1]) / std[1];
...
void launchResizeKernel(
    ...
    printf("In launchResizeKernel 1\n");
    printf("grid = %d\tthread = %d\n", blocks, kThreadsPerBlock);
    ...
    printf("In launchResizeKernel 2\n");
}
